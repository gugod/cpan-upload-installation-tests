...
do
    ...
    then
        ...
        curl --silent https://gugod.org/feed/cpanm-installation-notest-FAIL/items -X POST -H "Authentication: Bearer ${FEEDRO_TOKEN_NOTEST_FAIL}" -F id="$disturl" -F title="$distname" -F content_text='<'${dist_locallib}.log
    fi
done
