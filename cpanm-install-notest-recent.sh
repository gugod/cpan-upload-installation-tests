...
do
    ...
    then
        ...
        curl https://gugod.org/feed/cpanm-installation-notest-SUCCESS/items -X POST -H "Authentication: Bearer ${FEEDRO_TOKEN_NOTEST_SUCCESS}" -F id="$disturl" -F title="$distname" -F content_text='<'${dist_locallib}.log
    else
        fail=1
        ...
        curl https://gugod.org/feed/cpanw-installation-notest-FAIL/items -X POST -H "Authentication: Bearer ${FEEDRO_TOEKN_NOTEST_FAIL}" -F id="$disturl" -F title="$distname" -F content_text='<'${dist_locallib}.log
    fi
done
